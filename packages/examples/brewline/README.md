...
Generated by `@composoft/composer` against `@composoft/registry-example-postgres`.
...
- `lib/registry.ts` — re-exports the registry from `@composoft/registry-example-postgres`.
...
- [@composoft/registry-example-postgres] — the source of every block, adapter, workflow id this app uses.
